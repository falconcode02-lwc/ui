...
export class DashboardComponent implements AfterViewInit {
  ...
  constructor(
    ...
    private serviceWrkflw: WorkflowService,
  ...
  ngOnInit() {
    ...
    this.workspaceSub = this.contextService.workspace$.subscribe((wsId) => {
      this.currentWorkspace = wsId || "default";
  ...
  private refreshDashboard() {
    ...
    this.statusList.forEach((s) => (s.count = -1));
    this.loadCounts();
    this.loadWorkflows(1);
  }

  private loadCounts() {
    this.httpsService.getDashboardCount(this.currentWorkspace).subscribe({
      next: (a: any) => {
        let data = a || {};
        let total =
          (data.RUNNING || 0) +
          ...
          (data.TIMED_OUT || 0) +
          (data.CANCELED || 0) +
          (data.CONTINUED_AS_NEW || 0);

        this.statusList[0].count = total;
        this.statusList[1].count = data.RUNNING || 0;
        this.statusList[2].count = data.COMPLETED || 0;
        this.statusList[3].count = data.FAILED || 0;
        this.statusList[4].count = data.TERMINATED || 0;
        this.statusList[5].count = data.TIMED_OUT || 0;
        this.statusList[6].count = data?.CANCELED || 0;
        this.statusList[7].count = data?.CONTINUED_AS_NEW || 0;
      },
      error: () => {
        ...
        .subscribe(
          (d) => {
            ...
            if (d.nextPageToken) {
              ...
              try {
                this.tokensByPage[page + 1] = this.toBase64(
  ...
  private buildQuery(): string {
    ...
    if (this.selectedStatuses && this.selectedStatuses.length) {
      const statusParts = this.selectedStatuses.map(
        (s) => `ExecutionStatus=\"${s}\"`
      );
    ...
    if (this.selectedWorkflowType && this.selectedWorkflowType.trim()) {
      const wfTypes = this.selectedWorkflowType
        .split(",")
        .map((w) => w.trim())
        .filter(Boolean);
      if (wfTypes.length === 1) {
