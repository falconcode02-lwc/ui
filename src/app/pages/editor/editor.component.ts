...
export class EditorComponent implements OnInit {
  ...
  code: string =
    "// Select a function from the left panel to view/edit its code";
  ...
  ngOnInit() {
    ...
    this.actionService.registerAction({
      ...
      action: () => {
        window.open(
          environment.docUrl +
            "/docs/getting-started/code-editor/" +
            this.selectedFile.classType,
          "_blank"
        );
      },
    });
  ...
  onTabIndexChange(e: any) {
    ...
    if (this.selectedIndex == -1) {
      this.vis = Visibility.none;
      this.actionService.setEnabledDelayMultiple(
        ["compile", "deploy", "help"],
        false,
        0
      );
  ...
  handleNodeClick(node: any) {
    ...
    if (node) {
      if (node.level === 0) {
        ...
        if (
          node.classType == NodeType[NodeType.function] ||
          node.classType == NodeType[NodeType.model] ||
          node.classType == NodeType[NodeType.controller] ||
          node.classType == NodeType[NodeType.condition] ||
          node.classType == NodeType[NodeType.switch] ||
          node.classType == NodeType[NodeType.plugin]
        ) {
          this.actionService.setEnabledDelayMultiple(
            ["compile", "deploy", "help"],
            true,
        ...
        } else if (node.classType == NodeType[NodeType.object]) {
          ...
          setTimeout(() => {
            ...
            if (this.tabContent[node.className]?.resColumns) {
              ...
              this.dbComp.setColumns(
                this.tabContent[node.className]["resColumns"]
              );
            } else {
              this.httpService
                .getColumns(node.className)
                .subscribe((res: any) => {
  ...
  bindChild() {
    this.bindTreeView("");
    this.httpService
      .getDataFiles(this.selectedFolder?.classTypeKey)
      .subscribe((res: any) => {
...
@run public SwitchConditionResponse invoke(FRequest req) {
  ...
  handleDeployClick(compileType: any) {
    ...
    let loadingId = this.message.loading("Compiling...", {
      nzDuration: 0,
    }).messageId;

    if (compileType != "onlyCompile") {
      this.addInfo("Deploying...");
    }
  ...
  addInfo(line: string) {
    ...
    this.logs = [
      ...
      new Date().toLocaleDateString() +
        ` : <span  class='info'>INFO: ${line}</span><br/>`,
    ];
  }
  ...
  addError(line: string) {
    ...
    this.logs = [
      ...this.logs,
      new Date().toLocaleDateString() +
        ` : <span class='error'>ERROR: ${line}</span><br/>`,
    ];
  }
  ...
  addWarn(line: string) {
    this.logs = [
      ...this.logs,
      new Date().toLocaleDateString() +
        ` : <span class='warn'>WARN: ${line}</span><br/>`,
    ];
  ...
  onChange(value: string): void {
    this.filteredOptions = this.searchOptions.filter(
      (option: any) =>
        option.className.toLowerCase().indexOf(value.toLowerCase()) !== -1
    );
  ...
  onSelect(id: any) {
    console.log(id);
    debugger;
    let opt: any = this.searchOptions.find(
      (option: any) => option.className == id.nzValue
    );
