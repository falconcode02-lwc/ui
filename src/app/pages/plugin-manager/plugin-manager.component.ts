  ...
  implements OnInit, AfterViewInit, OnDestroy
{
  ...
  private syncFormFromCodeEditor(): void {
    ...
    const extractSetterRaw = (setter: string) => {
      ...
      if (setter == "Secrets" || setter == "Props") {
        ...
        const base64Match = code.match(
          new RegExp(
            `plugin\\s*\\.\\s*${methodName}\\s*\\(\\s*"([^"]*)"\\s*\\)`,
            "m"
          )
        );
        if (base64Match) {
      ...
      quotedMatch = code.match(
        new RegExp(
          `plugin\\s*\\.\\s*set${setter}\\s*\\(\\s*\\\"([\\s\\S]*?)\\\"\\s*\\)\\s*;`,
          "m"
        )
      );
      if (quotedMatch) {
    ...
    this.ngZone.run(() => {
      ...
      const isAiToolRaw = code.match(
        /plugin\s*\.\s*setAiTool\s*\(\s*(true|false)\s*\)/
      );
      ...
      .subscribe({
        next: (response: any) => {
          ...
          if (response && response.status === "Success") {
            ...
            this.message.success("Whoaaa! Compiled & Deployed successfully", {
              nzDuration: 5000,
            });
        ...
        error: (error: any) => {
          ...
          this.compileLogs.push(logMsg);
          this.message.error(
            "Failed to deploy plugin: " + (error.message || "Network error")
          );
          this.alldisabled(false);
          this.cdr.markForCheck();
        },
  ...
  generatePluginCode(): void {
    ...
    if (this.isEditMode && this.originalSourceCode) {
      this.generatedCode = this.updateInitSectionInCode(
   ...
   */
  private replacePluginSetter(
    code: string,
    setterSuffix: string,
    valueWithQuotes: string
  ): string {
  ...
  private replacePluginSetterRaw(
    code: string,
    setterSuffix: string,
    rawValue: string
  ): string {
    const pattern = new RegExp(
      `(plugin\\s*\\.\\s*set${setterSuffix}\\s*\\(\\s*)([\\s\\S]*?)(\\s*\\)\\s*;)`
    );
    if (pattern.test(code)) {
  ...
  ngOnInit(): void {
    ...
    this.pluginForm.get("isAiTool")?.valueChanges.subscribe((val: boolean) => {
      ...
      const replaced = current.replace(pattern, `$1${val}$3`);
      if (replaced !== current) {
        this.suppressAutoGenerateOnce = true;
        this.generatedCode = replaced;
        this.cdr.markForCheck();
  ...
  openEditDrawer(plugin: PluginDto): void {
    ...
    this.pluginForm.patchValue({
      ...
      isAiTool: (plugin as any).isAiTool || false,
  ...
  private fetchOriginalSourceCode(plugin: PluginDto): void {
    ...
    this.httpService.getDataByClassTypeAndFqcn<any>("plugin", fqcn).subscribe({
      next: (resp: any) => {
        ...
        this.originalSourceCode =
          raw || this.generateNewPluginCode(this.pluginForm.value);
      ...
      error: (err) => {
        ...
        this.originalSourceCode = this.generateNewPluginCode(
          this.pluginForm.value
        );
        this.generatedCode = this.originalSourceCode;
        this.isLoadingSourceCode = false;
      ...
      nzFooter: [
        ...
        },
        {
          ...
          onClick: (componentInstance) =>
            this.saveFormBuilderProps(componentInstance),
        },
      ],
    });
    ...
    if (this.formBuilderModalRef.componentInstance && initialData) {
      ...
      if (
        ...
        fbComponent.formConfig.set({
          ...
          successMessage:
            initialData.successMessage || "Form submitted successfully!",
          errorMessage:
          ...
          showResetButton:
            initialData.showResetButton !== undefined
              ? initialData.showResetButton
              : true,
          showSubmitButton:
            initialData.showSubmitButton !== undefined
              ? initialData.showSubmitButton
              : true,
  ...
  openSecretsBuilder(): void {
    ...
    if (secretsValue) {
      try {
        initialData = JSON.parse(secretsValue);
      } catch (e) {
    ...
    this.formBuilderModalRef = this.modal.create({
      ...
      nzFooter: [
        ...
        {
          label: "Save Secret Schema",
          type: "primary",
          onClick: (ci) => this.saveSecretsSchema(ci),
        },
      ],
    });
    if (this.formBuilderModalRef.componentInstance && initialData) {
  ...
  deletePlugin(id: number): void {
    const loadingMsg = this.message.loading("Deleting plugin...", {
      nzDuration: 0,
    });
        ...
        next: (response: any) => {
          ...
          if (response && response.status === "Success") {
            ...
            this.modal.create({
              ...
              nzContent: `<div style='margin-bottom:12px;'><b>Compile Output:</b><br/><pre style='white-space:pre-wrap;'>${
                this.compileResponseText
              }</pre></div><div><b>Source Code:</b><br/><textarea style='width:100%;height:300px;font-family:monospace;font-size:13px;'>${javaCode
                .replace(/</g, "&lt;")
                .replace(/>/g, "&gt;")}</textarea></div>`,
              nzWidth: "800px",
              nzOnOk: () => {
                this.compileResponseVisible = false;
              },
            });
          } else if (response && response.status === "Failed") {
            logMsg =
            ...
            this.compileResponseText =
              response.message || "Compilation completed with warnings.";
            this.compileResponseVisible = true;
            this.modal.warning({
  ...
  private generateInitSection(plugin: PluginDto): string {
    ...
    const isAiTool = (plugin as any).isAiTool || false;
    const aiToolDescription = (plugin as any).aiToolDescription || "";
  ...
  void init() {
      ...
      plugin.setAiTool(${isAiTool});
      plugin.setAiToolDescription("${this.escapeJavaString(
        aiToolDescription
      )}");
   ...
   */
  private updateInitSectionInCode(
    originalCode: string,
    plugin: PluginDto
  ): string {
    ...
    if (onloadIdx === -1) {
      console.warn(
        "Could not find @onload in original code, returning full code"
      );
      return this.generateFullPluginCode(plugin);
    }
    ...
    if (braceStart === -1) {
      console.warn(
        "Could not find opening brace for init(), returning full code"
      );
      return this.generateFullPluginCode(plugin);
    }
    ...
    if (braceEnd === -1) {
      console.warn(
        "Could not match closing brace for init(), returning full code"
      );
      return this.generateFullPluginCode(plugin);
    }
...
*/
  ...
  void registerPlugin() {
      ...
      plugin.setPluginDocument("${this.escapeJavaString(pluginDocument)}");
      plugin.setIcon("${this.escapeJavaString(icon)}");
      plugin.setAiTool(${(plugin as any).isAiTool || false});
      plugin.setAiToolDescription("${this.escapeJavaString(
        (plugin as any).aiToolDescription || ""
      )}");
      plugin.setProps(${propsFormatted});
      plugin.setSecrets(${secretsFormatted});
      plugin.setPluginType(PluginType.${
    ...
    reader.onload = (e: any) => {
      ...
      img.onload = () => {
        ...
        if (img.width !== 64 || img.height !== 64) {
          ...
          this.message.error(
            `Icon must be exactly 64x64 pixels! Current size: ${img.width}x${img.height}`
          );
          this.cdr.markForCheck();
          return;
        }
  ...
  exportPlugin(): void {
    ...
    a.href = url;
    a.download =
      (this.pluginForm.get("pluginName")?.value || "plugin") + ".ffx";
