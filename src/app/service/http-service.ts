...
export class HttpService {
  ...
  deleteAgent<T = any>(agentId: string): Observable<T> {
    return this.httpClient
      .delete<T>(
        `${this.baseUrl}/api/ai/agents/${encodeURIComponent(agentId)}`,
  ...
  testAPI<T = any>(params: {
    ...
    switch (method?.toUpperCase()) {
      ...
      case "PATCH":
        return this.httpClient
          .patch<T>(url, body ?? {}, { headers: httpHeaders })
          .pipe(catchError(this.handleError));
      case "DELETE":
        return this.httpClient
  ...
  getDataByClassTypeAndFqcn<T = any>(
    classType: string,
    fqcn: string
  ): Observable<T> {
    return this.get<T>("/getByClassTypeAndFqcn", {
      classType,
      fqcn,
    }) as Observable<T>;
  }
  ...
  executeWorkflow<T = any>(payload: any): Observable<T> {
    return this.post<T>(
      "/api/v1/workflowManager/createWorkflow",
      payload
    ) as Observable<T>;
  }
  ...
  executeWorkflowUpdateflow<T = any>(payload: any): Observable<T> {
    return this.post<T>(
      "/api/v1/workflowManager/updateWorkflow",
      payload
    ) as Observable<T>;
  }
  ...
  getColumns<T = any>(tableName: string): Observable<T> {
    return this.get<T>("/api/schema/getColumns", {
      tableName,
    }) as Observable<T>;
  }

  getDashboardCount<T = any>(namespace: string = "default"): Observable<T> {
    return this.get<T>("/api/v1/workflowManager/getAllCounts", {
      namespace,
    }) as Observable<T>;
  }

  // example: request history with default body observe
  getDashboardHistory<T = any>(
    workflowId: string,
    namespace: string = "default"
  ): Observable<T> {
    return this.get<T>("/getHistory", {
      workflowId,
      namespace,
    }) as Observable<T>;
  }
  ...
  messageAgent<T = any>(
    ...
    return this.post<T>(
      `/api/ai/agents/${encodeURIComponent(agentId)}/message`,
      body
    ) as Observable<T>;
  }
  ...
  getAgentMemory<T = any>(agentId: string): Observable<T> {
    return this.get<T>(
      `/api/ai/agents/${encodeURIComponent(agentId)}/memory`
    ) as Observable<T>;
  }
  ...
  callAgentN8n<T = any>(agentId: string, body: any): Observable<T> {
    return this.post<T>(
      `/api/ai/agents/${encodeURIComponent(agentId)}/tools/n8n`,
      body
    ) as Observable<T>;
  }
  ...
  getSecret<T = any>(id: number | string): Observable<T> {
    return this.get<T>(
      `/api/secrets/${encodeURIComponent(String(id))}`
    ) as Observable<T>;
  }
