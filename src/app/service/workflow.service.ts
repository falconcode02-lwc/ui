...
export class WorkflowService {
  ...
  private getContextQuery(hasExistingParams: boolean = false): string {
    const workspace = this.contextService.getWorkspace();
    const project = this.contextService.getProject();
    const params = new URLSearchParams();
    if (workspace) params.append("workspaceId", workspace);
    if (project) params.append("projectId", project);
    const query = params.toString();
    if (!query) return "";
    return (hasExistingParams ? "&" : "?") + query;
  }

  getAll(): Observable<Workflow[]> {
    return this.http.get<Workflow[]>(`${this.apiUrl}${this.getContextQuery()}`);
  }

  getById(id: number): Observable<Workflow[]> {
    return this.http.get<Workflow[]>(
      `${this.apiUrl}/${id}${this.getContextQuery()}`
    );
  }
  ...
  active(id: number, active: boolean): Observable<void> {
    return this.http.get<void>(
      `${this.apiUrl}/${id}/active/${active}${this.getContextQuery()}`
    );
  }
  ...
  create(data: Workflow): Observable<Workflow> {
    return this.http.post<Workflow>(
      `${this.apiUrl}${this.getContextQuery()}`,
      data
    );
  }
  ...
  getWorkflows(
    ...
    return this.http.get<any>(
      `${
        this.apiUrlWkflows
      }/list?namespace=${namespace}&query=${query}&pageSize=${pageSize}&nextPageToken=${nextPageToken}${this.getContextQuery(
        true
      )}`
    );
  }
  ...
  terminateWorkflows(workflowId: string, reason: string): Observable<any> {
    return this.http.get<any>(
      `${
        this.apiUrlWkflows
      }/terminate?workflowId=${workflowId}&reason=${reason}${this.getContextQuery(
        true
      )}`
    );
  }
  ...
  getWorkflowStepsStatus(
    ...
    return this.http.get<any>(
      `${this.apiUrlWkflows}/getStepsStatus?namespace=${namespace}&workflowId=${workflowId}`
    );
  }
  ...
  exportWorkflow(workflow: Workflow): void {
    const exportData: WorkflowExportData = {
      ...
      workflowJsonRaw:
        typeof workflow.workflowJsonRaw === "string"
  ...
  validateWorkflowImport(workflowData: any): Observable<ValidationResult> {
    ...
    try {
      ...
      if (workflowJsonRaw && workflowJsonRaw.nodes) {
        ...
        nodes.forEach((node: any) => {
          ...
          if (
            ...
            const pluginId =
              node.meta?.pluginData?.plugin_id ||
        ...
        if (result.missingCalls.length > 0) {
          result.warnings.push(
            `This workflow references ${result.missingCalls.length} call(s). Please ensure they exist in your environment.`
          );
        }

        if (result.missingPlugins.length > 0) {
          result.warnings.push(
            `This workflow uses ${result.missingPlugins.length} plugin(s). Please ensure they are installed.`
          );
        }
      }
      ...
      if (
        ...
        workflowData.metadata.name.trim() === ""
      ) {
      ...
      if (
        !workflowData.metadata?.code ||
        workflowData.metadata.code.trim() === ""
      ) {
